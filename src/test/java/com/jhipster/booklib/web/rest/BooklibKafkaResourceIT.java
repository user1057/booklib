...
class BooklibKafkaResourceIT {
    ...
    void setup() {
        ...
        BooklibKafkaResource kafkaResource = new BooklibKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/booklib-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/booklib-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
